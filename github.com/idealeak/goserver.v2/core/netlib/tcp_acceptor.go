...
import (
	...
	"github.com/idealeak/goserver.v2/core/utils"
)
		...
		case conn, ok := <-a.connChan:
			...
			if tcpconn, ok := conn.(*net.TCPConn); ok {
				...
				if a.sc.KeepAlive {
					...
					//					err := tcpkeepalive.SetKeepAlive(conn, a.sc.KeepAliveIdle, a.sc.KeepAliveCount, a.sc.KeepAlivePeriod)
					//					if err != nil {
					//						logger.Logger.Warnf("(a *TcpAcceptor) sessionRoutine SetKeepAlive err:%v", err)
					//					}
				}
			}
